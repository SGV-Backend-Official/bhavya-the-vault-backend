...
const pauseTournamentController = async (req, res) => {
  try {
    ...
    if (tournament.createdBy.toString() !== req.user.id) {
      return res.status(403).json({
        success: false,
        message: "Only host can pause timer",
      });
    }
    ...
    tournament.isTimerPaused = true;

    await tournament.save();

    return res.status(200).json({
      success: true,
      message: "Blind timer paused successfully",
      data: tournament,
    });
  } catch (error) {
...
export { pauseTournamentController };
